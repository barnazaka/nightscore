...
export const zkConfigPath = path.resolve(__dirname, '..', 'contract', 'managed', 'nightscore');

export async function loadContract() {
  const contractPath = path.join(zkConfigPath, 'contract', 'index.js');
  const NightScore = await import(pathToFileURL(contractPath).href);
  const compiledContract = (contracts as any).CompiledContract.make('nightscore', NightScore.Contract);
...
export function deriveKeys(seed: string) {
  ...
  const result = (hdWallet as any).hdWallet.selectAccount(0).selectRoles([Roles.Zswap, Roles.NightExternal, Roles.Dust]).deriveKeysAt(0);
...
export async function createWallet(seed: string) {
  ...
  const unshieldedWallet = UnshieldedWallet({
    networkId, indexerClientConnection: walletConfig.indexerClientConnection,
...
export function signTransactionIntents(tx: any, signFn: any, proofMarker: any): void {
  ...
  for (const segment of tx.intents.keys()) {
    ...
    if (!intent) continue;
    const cloned = (ledger.Intent as any).deserialize('signature', proofMarker, 'pre-binding', intent.serialize());
...
export async function createProviders(walletCtx: any) {
  ...
  const walletProvider = {
    ...
    async balanceTx(tx: any, ttl?: Date) {
      ...
      const signFn = (payload: any) => walletCtx.unshieldedKeystore.signData(payload);
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({ privateStateStoreName: 'nightscore-state', walletProvider } as any),
    publicDataProvider: indexerPublicDataProvider(CONFIG.indexer, CONFIG.indexerWS),
    zkConfigProvider: new NodeZkConfigProvider(zkConfigPath),
    proofProvider: httpClientProofProvider(CONFIG.proofServer, new NodeZkConfigProvider(zkConfigPath) as any),
    walletProvider,
